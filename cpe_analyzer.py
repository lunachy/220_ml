...
if __name__ == "__main__":
    ...
    if args.most_common:
        os.chdir(root_dir)
        ks = ['opsS', 'impsS', 'pes', 'pixs', 'apis', 'secsS']
        all_ks = sum(map(lambda x: list(x), [combinations(ks, i) for i in range(1, len(ks) + 1)]), [])
        ...
        for i in val_npzs_data:
            val_y_pred_accu = np.mean(np.array(i[1]) == val_y_true)
            log.info('%s %.5f' %(i[0][:-13], val_y_pred_accu))
        sys.exit()
