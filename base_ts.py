...
def get_first_month(data, test_size, multiplier):
    ...
    if not False in list(last_7):
        ...
        for _daytype in daytype:
            ratio = np.mean(data[data["weekday"] == _daytype]["Number"])*1.0/np.mean(data[data["weekday"] == baseline]["Number"])
                    ...
                    if df[df["Number"] == 0].shape[0] == df.shape[0]:
                        ...
                        if days <= 7:
                            ...
                            try:
                                ...
                                predictions, new_lowerbound, new_upperbound = run_ariam_pf(df, maxar, maxma, diffn, test_size)
                                ...
                                upperbound = yday[-1]
                                #status = check_status(threatNumber, lowerbound, upperbound)
                                predictions.extend([yday[12], lowerbound, upperbound, new_lowerbound, new_upperbound, date, _type])
