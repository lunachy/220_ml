...
def read_conf():
    ...
    es_ip = cfg.get('es', 'es_ip').strip()
    es_port = cfg.get('es', 'es_port').strip()
    ...
    _options['es_ip'] = es_ip
    _options['es_port'] = es_port
...
def get_es_data(date1, event, cal_table, options):
    ...
    es = Elasticsearch([{'host': options['es_ip'], 'port': int(options['es_port'])}])
    results = es.search(index='analytical_data_{}'.format(date1), q='EVENT_TYPE:{}'.format(event), size=10000)
...
if __name__ == "__main__":
    ...
    for _event, _cal_table in zip(events, options['cal_table_names']):
        try:
            get_es_data(yesterday, _event, _cal_table, options)
        except Exception, e:
            print _event, e
